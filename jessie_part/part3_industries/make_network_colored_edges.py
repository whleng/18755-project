...
def create_network_with_highlighted_edges(input_csv, highlight_skill, output_pdf="job_titles_network_with_highlighted_edges.pdf"):
    ...
    for index, row in df.iterrows():
        company_name = row['industry_name']
    ...
    for i, row1 in df.iterrows():
        for j, row2 in df.iterrows():
            ...
            if len(common_skills) > 4:  # Only add an edge if there are common skills
                G.add_edge(row1['industry_name'], row2['industry_name'], weight=len(common_skills)-4, common_skills=common_skills)
    ...
    # Title of the plot
    plt.title(f'Industries Network with Louvain Communities (Highlighted Skill: {highlight_skill})')
    ...
    # plt.savefig("industries_network_with_highlighted_edges.png", format="png")
    # plt.savefig("industries_network_with_highlighted_edges.jpeg", format="jpeg")
...
create_network_with_highlighted_edges(
    input_csv="nodes.csv",  # Your input CSV file
