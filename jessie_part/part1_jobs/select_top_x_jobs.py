...
job_counts = merged_postings['title'].value_counts()

# Get the top 100 job titles
top_100_jobs = job_counts.head(100).index

# Filter the merged dataframe to keep only the top 100 job titles
top_100_postings = merged_postings[merged_postings['title'].isin(top_100_jobs)]
...
top_100_postings_grouped = top_100_postings.groupby('title').agg({
    'job_id': 'first',  # Keep one job_id per title (any, since we're grouping)
