...
for i, row1 in df.iterrows():
    for j, row2 in df.iterrows():
        if i >= j:  # Avoid duplicate pairs (i, j)
            continue
        # Find common skills
        skills1 = set(re.split(r',\s*', row1['unique_skills']))
        skills2 = set(re.split(r',\s*', row2['unique_skills']))
...
plt.savefig("industries_network.pdf", format="pdf")

# Optionally, you can save it as PNG or JPEG as well
plt.savefig("industries_network.png", format="png")
plt.savefig("industries_network.jpeg", format="jpeg")
